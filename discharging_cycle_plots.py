...
print("UNDERSTANDING THE PATTERN IN DISCHARGING CYCLE DATA")
...
type1_features = ["Voltage_measured", "Current_measured", "Temperature_measured", "Current_load", "Voltage_load", "Time"]
...
for file in files:
    ...
    if set(type1_features).issubset(df.columns):
        print(f"Processing {file} (Type 1)")
        
        # Extract relevant columns and drop NaN values
        df = df[["Temperature_measured", "Current_load", "Voltage_load", "Time", "Voltage_measured", "Current_measured"]].dropna()
        ...
        current_data.extend(df["Current_load"].values)
        voltage_data.extend(df["Voltage_load"].values)
...
if len(temp_data) > 10 and len(time_data) > 10:
    ...
    axs[0].scatter(time_data, voltage_data, color="red", alpha=0.5, label="Voltage Load")
    axs[0].set_xlabel("Time")
    axs[0].set_ylabel("Voltage")
    axs[0].set_title("Voltage Measured vs. Time & Voltage Load vs. Time")
    axs[0].legend()
    axs[0].grid(True)
    
    axs[1].scatter(time_data, current_measured_data, color="green", alpha=0.5, label="Current Measured")
    axs[1].scatter(time_data, current_data, color="purple", alpha=0.5, label="Current Load")
    axs[1].set_xlabel("Time")
    axs[1].set_ylabel("Current")
    axs[1].set_title("Current Measured vs. Time & Current Load vs. Time")
    axs[1].legend()
    axs[1].grid(True)
